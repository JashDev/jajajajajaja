...
function putImageInCell(x, y) {
    ...
    image.src = isPlayerTurn ? "./assets/images/black.svg" : "./assets/images/red.svg"
...
function canPlayerWinInRow(row) {
    ...
    if (colorInRow.filter(color => color === 'black' ).length === 2 && colorInRow.filter(color => color === undefined).length === 1) {
...
function finishGame() {
    ...
    modalTitle.innerText = isPlayerWin ? `Felicidades ${localStorage.getItem('playerName')}` : `¡Oh no ${localStorage.getItem('playerName')}!`
    modalSubtitle.innerText = isPlayerWin ? '¡Ganaste el juego!' : '¡Perdiste el juego!'
